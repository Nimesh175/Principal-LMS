...
mock.onGet('/apps/invoice/invoices').reply(config => {
  ...
  const filteredData = data.invoicesNew
    .filter(
      invoice =>
  ...
  return [
    200,
    {
      allData: data.invoicesNew,
      invoicesNew: paginateArray(filteredData, perPage, page),
...
// ------------------------------------------------
mock.onGet(/\/api\/invoiceNew\/invoicesNew\/\d+/).reply(config => {
  // // Get event id from URL
  const invoiceId = Number(config.url.substring(config.url.lastIndexOf('/') + 1))

  const invoiceIndex = data.invoicesNew.findIndex(e => e.id === invoiceId)
  const responseData = {
    invoice: data.invoicesNew[invoiceIndex],
    paymentDetails: {
      totalDue: '$12,110.55',
...
// ------------------------------------------------
mock.onDelete('/apps/invoiceNew/delete').reply(config => {
  ...
  const invoiceIndex = data.invoicesNew.findIndex(t => t.id === invoiceId)
  data.invoicesNew.splice(invoiceIndex, 1)
...
// GET: Return Clients
// ------------------------------------------------
mock.onGet('/api/invoiceNew/clients').reply(() => {
